          ...
          if (processesResult !== undefined) {
            ...
            for (const process of runningProcesses) {
              ...
              if (result !== undefined) {
                ...
                processProfiles[process.id] = extractProfileVariant(result.executor_action);
              }
            }
            ...
            if (setupProcess && !runningProcessDetails[setupProcess.id]) {
              ...
              if (result !== undefined) {
                ...
                // Extract ProfileVariant from the executor_action
                processProfiles[setupProcess.id] = extractProfileVariant(result.executor_action);
              }
            }
            ...
            for (const process of processesResult) {
              if (!processProfiles[process.id]) {
                ...
                processProfiles[process.id] = extractProfileVariant(process.executor_action);
              }
            }
    ...
    const defaultFollowUpVariant = useMemo(() => {
      ...
      if (codingAgentProcesses.length > 0) {
        ...
        for (let i = codingAgentProcesses.length - 1; i >= 0; i--) {
          const profileVariant = attemptData.processProfiles[codingAgentProcesses[i].id];
          if (profileVariant) {
      ...
      return null;
    }, [attemptData.processes, attemptData.processProfiles, selectedAttempt?.profile, profiles]);
